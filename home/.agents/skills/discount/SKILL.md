...
この skill は価格と支払い条件を比較する。ツールを実際に試して導入可否を決める流れは trial skill が主になる。

## 聞くより並べる

割引が使えるかどうかをユーザーに質問して確かめない。学生ですか、事業者ですか、と聞かれるより、適用したときの価格と条件が並んでいるほうが早く選べる。質問するのは、答えで提示が大きく変わる 1 点だけにする。

学割・教職員割・事業者向けのような、ユーザーの属性で適用が決まる割引は、記憶や推測で可否を判定しない。`~/Code/nozomiishii/brain/brain/PROFILE.md` を読んで、在学状況や事業形態を確かめてから見立てを書く。ローカルファイルを 1 つ読むだけなので、急ぎの依頼でも省略しない。読めない環境では、属性は未確認と明記して進める。プロフィールの中身をこの skill 本文へ書き写さない。
...
カタログの正本は [sources.md](sources.md)。開いたリンクが切れていたり、プログラムが終了していたりしたら、その事実を回答で報告する。DNS エラーや 404 で開けなかったリンクも、切れたものとして扱う。開いていないリンクを、切れていなかったと報告しない。使える一覧サイトを新しく見つけた場合も回答で提案する。
