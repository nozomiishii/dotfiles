...
各サイトを `running | done | failed | timeout | skipped` で管理する。入力 URL が `null` のサイトは `skipped`、locator や URL の検証に失敗したサイトは `failed` にする。
...
- 6 回目でも `running` のサイトは `timeout` にする。途中まで取得できた本文を完了済みの回答として扱わない
- 完了したタブはそれ以上ポーリングしない
...
各タブから最後のアシスタントメッセージを抽出し、最終応答として以下の markdown を返す。概要と比較は `done` の回答だけを対象にする。`timeout` の本文を載せる場合は未完了の途中出力と明記する:
...
| サイト | 状態 |
|---|---|
| ChatGPT | done / failed（理由） / timeout（120 秒時点で実行中） / skipped（理由） |
| Claude | done / failed（理由） / timeout（120 秒時点で実行中） / skipped（理由） |
| Gemini | done / failed（理由） / timeout（120 秒時点で実行中） / skipped（理由） |
...
「概要」セクションは `done` の回答をすべて読み終えてから書くこと。投げて返ってきた順に書かず、完了分が出揃ってから統合的にまとめる。完了が 1 件もなければ概要を作らず、その旨を明記する。
...
- レートリミット・回答取得エラー → そのサイトは `failed` として残りでレポートを作成し、理由を回答状態に明記
- 入力 URL が `null` のサイト → ポーリング・抽出をスキップし、レポートの「## タブ URL」では `n/a (Stage 1 で失敗)` と記載
- 120 秒で完了しないサイト → `timeout` とし、レポートの回答状態と該当回答欄に未完了であることを明記
