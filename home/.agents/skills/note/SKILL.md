...
description: >-
  ...
  Claude Code で /note、Codex で $note と入力したとき、または「ノートに残して」「ノートにして」「メモしておいて」と言ったときに使用する。
...
- トピック名 + サマリーの形式
- スキルを呼び出した発話でトピックが指定されていれば、それを優先する
- 候補がなければ「特になし」で終了
...
git remote が `nozomiishii/brain` を指す clone をホストの project 一覧または既存の local clone から探し、その絶対パスを `REPO` とする。sibling の [wt SKILL.md](../wt/SKILL.md) を明示的に読む。clone が見つからなければ、その repo 準備手順を使う。Claude Code の branch 名は `note-<トピックの英訳 kebab-case>`、Codex は wt skill が `codex/note-<トピックの英訳 kebab-case>-<task suffix>` を作る。作成結果の worktree 絶対パスを `WT`、実際の branch 名を `BRANCH` として保持し、ホスト固有の配置場所から再構築しない。
...
```shell
# タイトル構成語、タグ、別名、日本語・英語の表記ゆれを別々の検索語にする
rg -li --glob '*.md' \
  -e "<タイトル構成語>" \
  -e "<タグ>" \
  -e "<別名や英訳>" \
...
検索語は 1 語に決め打ちしない。候補を重複排除し、新しく作ったノート自身を除外してから本文を読む。1 つの検索語が 0 件でも重複なしと判定しない。
...
確認用に、作成したノートを現在のホストで開ける Markdown リンクで出力する。

Claude Code では cwd 相対のリンクを使う。cwd は予測できないのでこう作る:
...
Codex では現在の response rules に従う。通常は `$WT` を展開した絶対パスを Markdown リンクの target にする。スペースを含む target は山括弧で囲む。
...
（日本語の PR 本文）
EOF
gh pr create -R nozomiishii/brain --base main --head "$BRANCH" --title "chore: add <topic> note" --body-file "$BODY_FILE"
```
