...
- デフォルトの projects.json が無い cloud セッションでは、add_repo で nozomiishii/workspaces を追加し、PROJECTS_JSON を clone 先のパスに読み替える。読めるのは push 済みの版である旨を対象リストの提示に添える
...
- ユーザーが OK を出したら次に進む
- cloud セッションでは、OK の後に対象 repo を親セッションでまとめて add_repo し、対象ファイル有無の check はその後に行う。有無が未確認のまま提示する表にはその旨を記す
