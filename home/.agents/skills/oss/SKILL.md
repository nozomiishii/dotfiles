...
- バグとして報告する症状は、他人のログを転記せず自分の環境で再現させる
- 再現に外部ツールの導入やユーザー環境の変更が要るときは、手順と撤去方法を示して承認を得る
- 再現できないときは、下書きに出典と未再現であることを書く
- コード変更を出すときは、修正前後の挙動を実環境で比べる
- 変更が既存の挙動に及ぶ範囲を実データで確かめる。変わらないことの確認も含む
...
- 英語で書く場合は会話内に和訳を全文併記する。要約や見出しだけの訳にしない。投稿本文には含めない
- 相手の実装を責める書き方にしない。今のコードになった経緯を時系列で示す
- 自分のプロジェクトの来歴や背景は書かない。相手が判断に使う事実だけ残す
