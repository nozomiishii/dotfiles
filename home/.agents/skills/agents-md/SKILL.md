...
- 候補の承認より前に AGENTS.md やスキルを編集しない。「整理して反映まで進めて」という依頼でも候補の提示を省略しない。
- 統合・退避で規則の意味が変わる・狭まる場合は、その候補に明記する。黙って意味を変えない。
- 矛盾は解決方向を自分で決めず、選択肢としてユーザーに出す。

確認不要と明示されている場合も候補は提示する。その場合だけ、承認を待たず同じターンで反映してよい。
...
- 候補は一覧にせず、AGENTS.md の「判断依頼」の型で 1 件ずつ出す。各候補に、対象ルール、種別 (削除・統合・退避・スキル化・矛盾・強調)、意味の変化を書く
- 承認された候補から順に反映する。修正指示があれば反映して実行する
- スキルの新規作成・編集を伴う候補は、Superpowers の writing-skills に従う
