...
- ユーザーに意思決定を求める論点は、**1 ターンにつき基本 1 つ**に絞る。ラリー回数が増えてもよい。並行開発でコンテキストスイッチしている最中に独立した論点を複数同時に提示されると認知負荷が高いため。これは prose での質問にも `AskUserQuestion` にも適用し、互いに独立した決定を同じターンに複数並べない。
