...
- ユーザーに意思決定を求める論点は、**1 ターンにつき基本 1 つ**に絞る。ラリー回数が増えてもよい。並行開発でコンテキストスイッチしている最中に独立した論点を複数同時に提示されると認知負荷が高いため。聞き方を問わず適用する。`AskUserQuestion`（選択肢 UI）でも、返答の文章で直接たずねる場合でも、互いに独立した決定を同じターンにまとめて並べないこと。
