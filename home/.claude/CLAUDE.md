...
- 例: Claude Code statusline の色を選ぶとき、`statusline.sh` の出力を一時的に複数行にし、各行で異なる色コード + ラベルを並べて出す。Claude Code を再描画すれば全候補を同時に見比べられる。決まったら本番形（1行）に戻す。
- 同様の手法は starship.toml、shell prompt、terminal config の調整にも使える。
