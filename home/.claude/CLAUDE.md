...
- worktree / マージ済みブランチの後始末を提案しない。「不要なら `git branch -d` / `git worktree remove` で消せます」のような促しはしないこと。マージ済み worktree・ブランチの削除は lefthook の post-merge（`nozo-git-harvest`）で自動化済みなので、ユーザーに掃除を促す必要はない。
...
- ユーザーに意思決定を求める論点は、**1 ターンにつき基本 1 つ**に絞る。ラリー回数が増えてもよい。並行開発でコンテキストスイッチしている最中に独立した論点を複数同時に提示されると認知負荷が高いため。
- 例外: その 1 つの決定に密接に依存する補足・選択肢は同じターンに含めてよい。自明な選択は論点にせず勝手に決めて進める（上記の「確認はしない」と同趣旨）。
...
- ユーザーが設定・挙動の存在を前提に「困ってる」と言った時、表層の grep やデフォルト知識で「そんな設定はありません」と早合点で否定しない。実装と公式ドキュメントを末端まで辿ってから是非を判断すること。`extends` / `import` / シンボリックリンク / パッケージ依存などの参照チェーンは末端まで辿る（直の設定ファイルだけ見て結論しない）。どうしても否定する場合も「どこまで確認したか」を明示し、ユーザーが反証しやすくする。本題（困りごとの解決）と別のところで「あるか無いか」の押し問答になり消耗するのを避けるため。
