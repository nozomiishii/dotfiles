  ...
  "SessionStart": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.hooks/apply-direnv.sh",
          "timeout": 300
        }
      ]
    }
...
- SessionStart / CwdChanged: `~/.hooks/apply-direnv.sh` で repo root の `.envrc` を direnv 経由で評価する。非対話シェルでは direnv の prompt hook が発火しないための配線（[dotfiles#1268](https://github.com/nozomiishii/dotfiles/issues/1268)）。副作用（依存 install / worktree の ff 追従）を実行し、`CLAUDE_ENV_FILE` に env を注入する。CwdChanged により別 repo へ移動したとき env が入れ替わる。timeout 300 は fresh worktree の初回 install（数十秒）対策。Codex は `~/.codex/hooks.json` の SessionStart から同じスクリプトを呼ぶ（CLAUDE_ENV_FILE が無いため副作用のみ）
