  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.hooks/block-op-cli.sh",
          "timeout": 5
        },
        {
          "type": "command",
          "command": "bash ~/.hooks/block-main-head-rewrite.sh",
          "timeout": 10
        },
        {
          "type": "command",
          "command": "bash ~/.hooks/inspect-git-push.sh",
          "timeout": 10
        }
      ]
    },
...
- PreToolUse (`matcher: "Bash"`): `~/.hooks/block-op-cli.sh` で Bash コマンド文字列内の `op` CLI パターンを検出・ブロック。deny リストでは防げない間接実行（`bash -c "op ..."`, `sh -c "op ..."` 等）への二重防御。CVE-2025-55284 等の prompt injection 攻撃対策。スクリプトは Claude/Codex 共通のため `~/.hooks/` に置き、Codex は `~/.codex/hooks.json` から同じものを呼ぶ
- PreToolUse (`matcher: "Bash"`): `~/.hooks/block-main-head-rewrite.sh` で、HEAD がリモート default branch に含まれる（= ブランチに自分のコミットが無い）状態での `git commit --amend` / commit-ish 指定の `git reset` を deny。commitlint 失敗後の誤 amend が main 先端コミットの差分を巻き込み、PR に無関係な変更が混入した事故（[#1327](https://github.com/nozomiishii/dotfiles/pull/1327), [#1328](https://github.com/nozomiishii/dotfiles/pull/1328)）の再発防止
- PreToolUse (`matcher: "Bash"`): `~/.hooks/inspect-git-push.sh` で、`git push` 前に `origin/HEAD..HEAD` のコミット一覧を `additionalContext` として注入。別タスクのコミットが同じブランチに積まれたまま push される混入（[#1327](https://github.com/nozomiishii/dotfiles/pull/1327) の副次事故）を push 直前に気づけるようにする。ブロックはしない
...
- `.envrc` 評価（SessionStart / CwdChanged の apply-direnv.sh）はグローバルではなく各 repo の `.claude/settings.json` + `.hooks/apply-direnv.sh` に置く（[dotfiles#1268](https://github.com/nozomiishii/dotfiles/issues/1268)）。グローバル配線だとチームメイト全員の `~/.claude/settings.json` への配布が必要になるため、repo を clone すれば効く形にしている。Codex も各 repo の `.codex/hooks.json` が同じスクリプトを呼ぶ（CLAUDE_ENV_FILE が無いため副作用のみ）
...
`"none"` を選んだ理由は、worktree のセットアップ（`.hooks/apply-direnv.sh` による `.envrc` 評価）が `SessionStart` hook 頼みのため。`SessionStart` の source は `startup` / `resume` / `clear` / `compact` の 4 つだけで `worktree` は存在せず、background セッションが稼働中に `EnterWorktree` で遅延的に worktree へ移っても `SessionStart` は再発火しない。つまりデフォルトの `"worktree"` だと、新規 worktree に `node_modules` が無いまま整わない可能性がある（`CwdChanged` が発火すれば `.envrc` 評価で整うはずだが未検証）。`"none"` にすれば background は常に本体（起動時に SessionStart hook が `.envrc` を評価して整えた状態）で動くため、この問題が起きない。
